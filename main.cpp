...
typedef GLfloat Matriz[16];
Matriz rotCubo[2];

bool aberto = FALSE;

void Door_One_Frontal(float x, float y, float z){
    glBegin(GL_QUADS);

    /* Maçaneta */
    glColor3f( 0.6, 0.45, 0.05);
        glVertex3f (x+1, 6+y, z);
        glVertex3f (x+1.5, 6+y, z);
        glVertex3f (x+1.5, y+7, z);
        glVertex3f (x+1, y+7, z);

    /* Estrutura */
    glColor3f( 0.9, 0.9, 0.9);
        glVertex3f (x, y, z);
        glVertex3f (x+0.25, y, z);
        glVertex3f (x+0.25, y+13.5, z);
        glVertex3f (x, y+13.5, z);

    /* Vidro */
    glColor4f( 0, 0.5, 1.0, 0.7f );

        glVertex3f (x, y, z);
        glVertex3f (x+4.75, y, z);
        glVertex3f (x+4.75, y+13.5, z);
        glVertex3f (x, y+13.5, z);

    glEnd();
}

void Door_Two_Frontal(float x, float y, float z){
    glBegin(GL_QUADS);
...
void Door_One_Lateral(float x, float y, float z){
    glBegin(GL_QUADS);
    ...
    glColor3f( 0.6, 0.45, 0.05);
        ...
        glVertex3f (x, y+7, z+1.5);
        glVertex3f (x, y+7, z+1);

    /* Estrutura */
    glColor3f( 0.9, 0.9, 0.9);
        glVertex3f (x, y, z);
        glVertex3f (x, y, z+0.25);
        glVertex3f (x, y+13.5, z+0.25);
        glVertex3f (x, y+13.5, z);
    ...
    glColor4f( 0, 0.5, 1.0, 0.7f );
        ...
        glVertex3f (x, y, z);
        glVertex3f (x, y, z+4.75);
        glVertex3f (x, y+13.5, z+4.75);
        glVertex3f (x, y+13.5, z);

    glEnd();
}

void Door_Two_Lateral(float x, float y, float z){
    glBegin(GL_QUADS);
...
void matrizIdentidade(Matriz A){
    int i,j;
    for (i=0; i<4; i++)
        for (j=0; j<4; j++) {
            if (i==j)
...
void rotacao(float x, float y, float z){
    glPushMatrix();
        ...
        glTranslatef(-x,0,-z);
        glPushMatrix();
            glMultMatrixf(rotCubo[0]);
            glGetFloatv(GL_MODELVIEW_MATRIX, rotCubo[0]);
        glPopMatrix();
    glPopMatrix();
    aberto = !aberto;
}
...
void Glass_Triangle(float x, float y, float z, double i, double j)
{
    glColor4f( 0, 0.5, 1.0, 0.7f );
...
void Wall_Trapeze1(float x, float y, float z, double i, double j, double k, float *color)
{
    glColor3f(color[0], color[1], color[2]);
    glBegin(GL_QUADS);
        glVertex3f (x-k, y, z-0.125);
        glVertex3f (x, y+i, z-0.125);
        glVertex3f (x+j, y+i, z-0.125);
        glVertex3f (x+j, y, z-0.125);
        glVertex3f (x, y, z-0.125);
    glEnd();
...
void Wall_Quadrado(float x, float y, float z, double i, double j, float *color)
{
    glColor3f(color[0], color[1], color[2]);
    glBegin(GL_QUADS);
        glVertex3f (x, y+i, z-0.125);
...
void Wall_Triangle(float x, float y, float z, double i, double j, float *color)
{
    glColor3f(color[0], color[1], color[2]);
    glBegin(GL_TRIANGLES);
        glVertex3f (x, y, z-0.125);
 ...
 */
        ...
        /*-----------------cobertura da cozinha do segundo andar-----------------------*/
        glPushMatrix();
			glColor3f(0.7, 0.7, 0.7 );
			glTranslatef(57.2, 23.3, 29.1);  //ou 9.5
			glScalef(35.5, 0.15, 32.0);   //alterado
			glutSolidCube(1.0);
		glPopMatrix();
        /*----------------------ESCADA-------------------------*/
		...
		Wall(14.65, WALL_FIRST_FLOOR, 45.0, 19.3, WALL_HEIGHT-3, WALL_WIDTH, 0, color[0]);
		Wall(28.8, WALL_FIRST_FLOOR-3.5, 45.0, 10, WALL_HEIGHT-10, WALL_WIDTH, 0, color[0]);
		Wall(36.6, WALL_FIRST_FLOOR, 45.0, 6.0, WALL_HEIGHT-3, WALL_WIDTH, 0, color[0]);
		Wall(44.3, WALL_FIRST_FLOOR-3.5, 45.0, 9.5, WALL_HEIGHT-10, WALL_WIDTH, 0, color[0]);
		Wall(62.0, WALL_FIRST_FLOOR, 45.0, 26.0, WALL_HEIGHT-3, WALL_WIDTH, 0, color[0]);

		/*estrutura de vidro da parede*/
		Wall(27.5, WALL_FIRST_FLOOR+2.5, 45.0, 0.5, WALL_HEIGHT/2 - 0.5 , WALL_WIDTH, 0, color[0]);
		Wall(30.5, WALL_FIRST_FLOOR+2.5, 45.0, 0.5, WALL_HEIGHT/2 - 0.5 , WALL_WIDTH, 0, color[0]);
		Wall(43.0, WALL_FIRST_FLOOR+2.5, 45.0, 0.5, WALL_HEIGHT/2 - 0.5 , WALL_WIDTH, 0, color[0]);
		Wall(46.0, WALL_FIRST_FLOOR+2.5, 45.0, 0.5, WALL_HEIGHT/2 - 0.5 , WALL_WIDTH, 0, color[0]);
		Wall(29.0, WALL_FIRST_FLOOR+2.5, 45.0, 9.5, 0.5, WALL_WIDTH, 0, color[0]);
        Wall(44.3, WALL_FIRST_FLOOR+2.5, 45.0, 9.5, 0.5, WALL_WIDTH, 0, color[0]);
        ...
        /*-------------cozinha segundo andar---------------------------------------------------*/
        Wall(57.2, WALL_SECOND_FLOOR-4.2, 13.2, 35.5, WALL_HEIGHT/2-1.5, WALL_WIDTH, 0, color[5]);
        Wall(41.0, WALL_SECOND_FLOOR+1.55, 13.2, 3.0, WALL_HEIGHT/2-2.0, WALL_WIDTH, 0, color[5]);
        Wall(73.5, WALL_SECOND_FLOOR+1.55, 13.2, 3.0, WALL_HEIGHT/2-2.0, WALL_WIDTH, 0, color[5]);
        Wall(39.5, WALL_SECOND_FLOOR+5.3, 41.5, WALL_WIDTH, 2.0, 7.0, 0, color[5]);
        Wall(57.3, WALL_SECOND_FLOOR+5.3, 13.2, 35.5, 2.0, WALL_WIDTH, 0, color[5]);
        Wall(39.5, WALL_SECOND_FLOOR-0.5, 25.5, WALL_WIDTH, WALL_HEIGHT-1.5, 25.0, 0, color[5]);

		//Wall(45.0, WALL_SECOND_FLOOR+0.4, 25.2, 35.0, WALL_HEIGHT-1.5, WALL_WIDTH, 0, color[5]);
        ...
        /* Estrutura de Vidro lateral direita - II*/
        Wall(-10.0, WALL_SECOND_FLOOR, 45.0, 0.5, WALL_HEIGHT+29.5, WALL_WIDTH, 0, color[0]);
        Wall(-5.0, WALL_SECOND_FLOOR, 45.0, 0.5, WALL_HEIGHT+31.5, WALL_WIDTH, 0, color[0]);
        Wall(0.0, WALL_SECOND_FLOOR, 45.0, 0.5, WALL_HEIGHT+33, WALL_WIDTH, 0, color[0]);

        for (int i = 0; i < 6; i++)
            Wall(-5.0, WALL_SECOND_FLOOR+21-i*7, 45.0, 20.0, 0.5, WALL_WIDTH, 0, color[0]);
        ...
        for (int i = 1; i <14; i++)
            Wall(-65.0+i*5, WALL_SECOND_FLOOR+3.8+i*0.7, -45.0, 0.5, WALL_HEIGHT/2+i*1.4, WALL_WIDTH, 0, color[0]);

        Wall(5, WALL_SECOND_FLOOR+12.9, -45.0, 0.5, WALL_HEIGHT/2+19, WALL_WIDTH, 0, color[0]);
        Wall(10, WALL_SECOND_FLOOR+12.9, -45.0, 0.5, WALL_HEIGHT/2+19, WALL_WIDTH, 0, color[0]);
        Wall(15, WALL_SECOND_FLOOR+12.9, -45.0, 0.5, WALL_HEIGHT/2+19, WALL_WIDTH, 0, color[0]);
        Wall(20, WALL_SECOND_FLOOR+12.9, -45.0, 0.5, WALL_HEIGHT/2+19, WALL_WIDTH, 0, color[0]);
        ...
        Wall(-7.5, WALL_FIRST_FLOOR+29, -45.0, 55.0, 0.5, WALL_WIDTH, 0, color[0]);
        Wall(0, WALL_FIRST_FLOOR+36, -45.0, 40.0, 0.5, WALL_WIDTH, 0, color[0]);
       ...
       // Wall(-65, WALL_FIRST_FLOOR, 45.0, 20.0, 0.5, WALL_WIDTH, 0, color[0]);
        ...
        for (int i = 0; i < 3; i++)
            Glass_One(-10.0+i*5, -3.7+6*7.0, 45.0, 5, (i+1));

        for (int i = 0; i < 4; i++)
            Glass_Triangle(-10.0+i*5, -3.7+6*7+i, 45.0, 1.0, 5);

        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 2; j++)
            {
                Glass_One(24.0+i*3.5,0.8+j*3.6, 45.0, 3.5, 3.6);
                Glass_One(39.0+i*3.5,0.8+j*3.6, 45.0, 3.5, 3.6);
            }
        ...
        for (int i = 8; i < 19; i++)
            Glass_One(-75.0+i*5, -3.7+4*7.0, -45.0, 5, 6.5);

        for (int i = 3; i < 8; i++){
            Glass_One(-75.0+i*5, -3.7+4*7.0, -45.0, 5, i-1.5);
            Glass_Triangle(-75.0+(i+1)*5, -3.7+4*7+i-1.5, -45.0, 1.3, 5);
        }

        for (int i = 12; i < 19; i++)
            Glass_One(-75.0+i*5, -3.7+5*7.0, -45.0, 5, 6.5);

        for (int i = 9; i < 12; i++){
            Glass_One(-75.0+i*5, -3.7+5*7.0, -45.0, 5, i-5.5);
            Glass_Triangle(-75.0+(i+1)*5, -3.7+5*7+i-5.5, -45.0, 1.3, 5);
        }

        for (int i = 13; i < 17; i++)
            Glass_One(-75.0+i*5, -3.7+6*7.0, -45.0, 5, i-12.5+pow(1.2,i-13));

        for (int i = 10; i < 13; i++)
            Glass_Triangle(-75.0+(i+3)*5, -3.7+6*7+i-11+pow(1.2,i-10), -45.0, 1.3, 5);

        Glass_One(10, -3.7+6*7.0, -45.0, 5, 5.5);
        Glass_One(15, -3.7+6*7.0, -45.0, 5, 5.5);
        ...
        /* Porta Lateral Direita com efeito para abrir e fechar*/
            glPushMatrix();
                glMultMatrixf(rotCubo[0]);
                Door_One_Frontal(-65.0, -3.7, 45.0);
            glPopMatrix();
            Door_Two_Frontal(-65.0, -3.7, 45.0);

         /* Porta Frontal das divisorias*/
            for (int i = 0; i < 3; i++){
                Door_One_Lateral(27, -3.7, 17.5-i*19.5);
                Door_Two_Lateral(27, -3.7, 17.5-i*19.5);
            }
        ...
        /* TERCEIRO ANDAR */
        Wall_Trapeze1(-32.5, WALL_SECOND_FLOOR+10.5, 45.0, 3.5, 17.5, 15.1,color[5]);
        Wall_Quadrado(-47.6, WALL_SECOND_FLOOR+7.5, 45.0, 3, 32.6,color[5]);
        Wall_Quadrado(-55, WALL_SECOND_FLOOR+7.5, 45.0, 1.8, 7.4,color[5]);
        Wall_Trapeze1(-19.5, WALL_SECOND_FLOOR+14.0, 45.0, 5.0, 4.5, 13.1,color[5]);
        Wall_Triangle(-15.0, WALL_SECOND_FLOOR+19.0, 45.0, 1.3, 4.5,color[5]);
        Wall_Triangle(-47.6, WALL_SECOND_FLOOR+9.3, 45.0, 1.2, 7.4,color[5]);

        glPushMatrix();
            glRotatef(180, 0, 1, 0);
                Wall_Trapeze1(-59.5, WALL_SECOND_FLOOR+12.1, 52.0, 6.5, 39.5, 15.1,color[0]);
                Wall_Quadrado(-74.7, WALL_SECOND_FLOOR+7.1, 52.0, 5, 54.7,color[0]);
                Wall_Trapeze1(-43.0, WALL_SECOND_FLOOR+18.6, 52.0, 6.0, 23.0, 16.2,color[0]);
                Wall_Trapeze1(-27.3, WALL_SECOND_FLOOR+24.6, 52.0, 3.0, 7.3, 16.0,color[0]);
        glPopMatrix();

        glPushMatrix();
            glRotatef(180, 0, 1, 0);
            glTranslatef(0.0, 0.0, -97.0);
                Wall_Trapeze1(-59.5, WALL_SECOND_FLOOR+12.2, 52.0, 6.5, 54.5, 15.1,color[0]);
                Wall_Quadrado(-74.7, WALL_SECOND_FLOOR+7.2, 52.0, 5, 69.7,color[0]);
                Wall_Trapeze1(-43.0, WALL_SECOND_FLOOR+18.7, 52.0, 6.0, 33.0, 16.2,color[0]);
                Wall_Trapeze1(-27.3, WALL_SECOND_FLOOR+24.7, 52.0, 3.0, 17.3, 16.0,color[0]);
        glPopMatrix();

        Wall_Quadrado(5.0, WALL_SECOND_FLOOR+18.7, 45.0, 9, 5,color[0]);
        Wall_Triangle(10.0, WALL_SECOND_FLOOR+27.7, 45.0, 0.5, 5,color[0]);

        Wall(20.0, 34.3, -48.5, WALL_WIDTH, WALL_HEIGHT+5.5, 7, 0, color[0]);
...
void init(void) {
    ...
    matrizIdentidade(rotCubo[0]);
    matrizIdentidade(rotCubo[1]);
...
int main(int argc, char** argv) {
    ...
    GLfloat ambientLight[] = {0.1f, 0.1f, 0.1f, 1.00f};
